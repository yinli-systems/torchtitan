...
"""GPU numerics test for CSA's Attention Gym ``gather_attn`` core.
...
``attn_gym.sparse.gather_attn.gather_attn`` directly. This test
...
def _prior_block_mask_forward(
    ...
    """Inline copy of the pre-gather_attn CSA forward (ratio == 4).

    Mirrors what ``DSV4FlexInnerAttention._forward_impl`` did for compress_ratio
    == 4 before ``CompressedSparseAttention`` was switched to call
    ``gather_attn`` directly: concatenate [swa_k, cmp_k, sink] into
...
class TestCSASelectedAttentionMatchesBlockMask(unittest.TestCase):
    ...
    def test_forward_and_grad_match_fp32(self):
        ...
        dsa = DSV4FlexInnerAttention(cfg)
