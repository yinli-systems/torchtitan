...
class DSV4FlexInnerAttention(FlexInnerAttention):
    ...
    ``gather_attn`` instead of building a ``BlockMask``.
    ...
    def build_block_mask(
        ...
        *,
        ...
        """Build the static SWA/HCA mask without token-pair-sized tensors.

        Block metadata conservatively contains every block touched by a query
        block. ``mask_mod`` applies the exact token-level static rules.
        """
        ...
        with spmd.no_typecheck():
            num_q_blocks = seqlen // bq
            num_kv_blocks = (kv_len + bk - 1) // bk
            q_block_idx_Q1 = torch.arange(num_q_blocks, device=device).unsqueeze(1)
            kv_block_idx_1K = torch.arange(num_kv_blocks, device=device).unsqueeze(0)
            q_start_Q1 = q_block_idx_Q1 * bq
            q_end_Q1 = (q_start_Q1 + bq - 1).clamp_max(seqlen - 1)
            kv_start_1K = kv_block_idx_1K * bk
            kv_end_1K = (kv_start_1K + bk - 1).clamp_max(kv_len - 1)

            window_start_Q1 = (q_start_Q1 - self.window_size + 1).clamp_min(0)
            window_blocks_QK = (
                (kv_start_1K < seqlen)
                & (kv_end_1K >= window_start_Q1)
                & (kv_start_1K <= q_end_Q1)
            )
            if self.compress_ratio > 1:
                num_visible_cmp_Q1 = (q_end_Q1 + 1) // self.compress_ratio
                compressed_blocks_QK = (
                    (num_visible_cmp_Q1 > 0)
                    & (kv_end_1K >= seqlen)
                    & (kv_start_1K < seqlen + num_visible_cmp_Q1)
                )
            else:
                compressed_blocks_QK = torch.zeros_like(window_blocks_QK)
            sink_blocks_1K = (kv_start_1K <= sink_idx) & (kv_end_1K >= sink_idx)
            selected_blocks_QK = (
                window_blocks_QK | compressed_blocks_QK | sink_blocks_1K
            )

            kv_num_blocks_BHQ = selected_blocks_QK.sum(dim=-1, dtype=torch.int32)[
                None, None
            ]
            kv_indices_BHQK = torch.argsort(
                selected_blocks_QK.to(torch.int32),
                dim=-1,
                descending=True,
                stable=True,
            ...
            def dsv4_mask_mod(
                ...
                if compress_ratio > 1:
                    compressed_mask = (
                        (kv_idx >= seqlen)
                        & (kv_idx < sink_idx)
                        & (kv_idx - seqlen < (q_idx + 1) // compress_ratio)
                    )
                else:
                    compressed_mask = torch.zeros_like(window_mask)
                return window_mask | compressed_mask | (kv_idx == sink_idx)

            return BlockMask.from_kv_blocks(
                kv_num_blocks_BHQ,
                kv_indices_BHQK,
                BLOCK_SIZE=(bq, bk),
                mask_mod=dsv4_mask_mod,
    ...
    def _forward_impl(
        ...
        *,
        ...
        elif isinstance(attention_masks, BlockMask):
            block_mask = attention_masks
        else:
            raise TypeError(
                "DSV4FlexInnerAttention attention_masks must be a BlockMask or None, "
...
class CompressedSparseAttention(DSV4FlexInnerAttention):
    """CSA core built on Attention Gym's ``gather_attn`` primitive.

    Unlike the other DSV4FlexInnerAttention subclasses, CSA does not concatenate
    ...
    ``DSV4FlexInnerAttention._forward_impl`` / ``_build_block_mask``). Instead it
    calls ``attn_gym.sparse.gather_attn.gather_attn`` directly:
    ...
    def forward(  # pyrefly: ignore[bad-param-name-override]
        ...
        *,
        ...
        with spmd.no_typecheck():
            ...
            # gather_attn indexes sparse_kv (cmp_k) directly, so unlike
            # DSV4FlexInnerAttention._forward_impl there is no "+ seqlen"
            ...
            # gather_attn wants [B, H, T, K]; local_kv/sparse_kv get a
            ...
            out_bhtk = gather_attn(
                ...
                scale=self.softmax_scale,
                impl="fused" if q.device.type == "cuda" else "reference",
            )
